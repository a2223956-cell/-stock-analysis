...
def fetch_board_info(code):
    """东财板块信息"""
    url = f"https://datacenter-web.eastmoney.com/api/data/v1/get?reportName=RPT_F10_CORETHEME_BOARDTYPE&columns=SECUCODE,SECURITY_NAME_ABBR,BOARD_CODE,BOARD_NAME,IS_PRECISE,BOARD_RANK,BOARD_TYPE&filter=(SECUCODE=%22{code}.{'SZ' if code.startswith(('0','3')) else 'SH'}%22)&pageSize=50"
    try:
...
def compute_scores(indicators, fund_flow, quote):
    ...
    if indicators.get("bounce_pct", 0) > 3:
        vol_score += 0.5
    vol_score = min(vol_score, 3.0)
    scores["量价关系"] = {"score": vol_score, "max": 3.0, "weight": 0.30}
    ...
    if fund_flow:
        ...
        if fund_flow and fund_flow[-1].get("main_net", 0) > 0:
            fund_score += 1.0
    fund_score = min(fund_score, 2.5)
    scores["资金流向"] = {"score": fund_score, "max": 2.5, "weight": 0.25}
    ...
    scores["均线技术"] = {"score": tech_score, "max": 2.0, "weight": 0.20}
    ...
    if 0 < pe < 30:
        ...
        val_score += 0.5
    val_score = min(val_score, 1.5)
    scores["估值安全"] = {"score": val_score, "max": 1.5, "weight": 0.15}

    # 维度5: 消息面 (10%)
    news_score = 0.5  # 默认中性
    scores["消息面"] = {"score": news_score, "max": 1.0, "weight": 0.10}

    # 加权总分
    total_weighted = sum(s["score"] * s["weight"] / s["max"] for s in scores.values() if s["max"] > 0)
    total_raw = sum(s["score"] for s in scores.values())

    return {
        "dimensions": scores,
        "total_raw": round(total_raw, 2),
        "total_weighted": round(total_weighted * 5, 2),  # 换算到5分制
...
def main():
    ...
    if klines and len(klines) >= 20:
        ...
        print(f"  缩量比: {indicators.get('shrink','?')} | 振幅: {indicators.get('kline',{}).get('body_ratio','?')}%")
    else:
    ...
    if indicators and quote:
        ...
        print(f"  5分制总分: {scores['total_weighted']:.2f}/5.0")
    else:
        print(f"  ✗ 无法评分(数据不足)")
